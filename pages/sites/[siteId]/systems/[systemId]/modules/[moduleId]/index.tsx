...
const ModulePage: NextPage = () => {
  ...
  const { siteId, systemId, moduleId } = router.query;
  const [activeTab, setActiveTab] = useState<Tab>('overview');

  // Handle loading state
  if (!siteId || !systemId || !moduleId) {
    return (
  ...
  const formatModuleName = (name: string) => {
    return name.split('-').map(part => 
      part === 'module' ? 'Module' : part
    ).join('-');
  };
  const moduleName = formatModuleName(String(moduleId));
  ...
  return (
    <div className="dashboard-grid">
      ...
      <aside className="dashboard-grid__sidebar">
        <Sidebar activePath={`/sites/${siteId}/systems/${systemId}/modules/${moduleId}`} />
      </aside>
      <main className="dashboard-grid__main">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm px-2 py-0.5 bg-gray-100 rounded">Module {moduleVersion}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-12">
                    <div>
                      ...
                      <div className="mt-1">
                        <div className="text-[#0066FF] font-medium bg-white px-3 py-1 rounded border border-[#0066FF]">Automatic</div>
                      </div>
                    </div>
                    <div>
